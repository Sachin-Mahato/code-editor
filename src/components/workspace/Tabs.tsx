...
// Language-specific icons (same as before)
const getLanguageIcon = (language: string) => {
    ...
    return (
        iconMap[language as keyof typeof iconMap] || (
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
...
const Tabs = () => {
    const { tabs, closeTab, switchTab, activeTab } = useFileContext()
        ...
        <div className="flex items-center bg-[#2d2d30] h-8">
            <ScrollArea className="flex-1">
                <div className="flex items-center min-w-max">
                    {tabs.map((tab) => (
                        <div
                            key={tab.fileId}
                            className={`
                relative flex items-center gap-2 px-3 py-1.5 border-r border-gray-700 cursor-pointer
                transition-all duration-200 min-w-[100px] max-w-[180px] flex-shrink-0
              ...
              `}
                            ...
                            onMouseEnter={() => setHoveredTab(tab.fileId)}
                            onMouseLeave={() => setHoveredTab(null)}
                        >
                            <div className={getLanguageColor(tab.language)}>{getLanguageIcon(tab.language)}</div>

                            <span className="text-xs font-medium truncate flex-1">{tab.fileName}</span>

                            {tab.isModified && <div className="w-1.5 h-1.5 bg-white rounded-full flex-shrink-0" />}

                            <Button
                                variant="ghost"
                                size="sm"
                                className={`
                  h-3.5 w-3.5 p-0 text-gray-400 hover:text-white hover:bg-gray-600 rounded-sm flex-shrink-0
                  ${hoveredTab === tab.fileId ? "opacity-100" : "opacity-0"}
